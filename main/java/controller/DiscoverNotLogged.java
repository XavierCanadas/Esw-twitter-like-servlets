	...
	protected void doGet(jakarta.servlet.http.HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (TweetRepository tweetRepository = new TweetRepository()) {
			try(UserRepository userRepository = new UserRepository()){
				...
				List<Tweet> tweetList = tweetService.getLatestTweets();
				
				UserService userService = new UserService(userRepository);
				List<User> userList = userService.getMostPopularUsers();

				request.setAttribute("latestTweets", tweetList);
				request.setAttribute("popularUsers", userList);
